...
//         let mut manager = self.app_handle.lock().as_ref().unwrap().global_shortcut();
...
//         let app_handle = self.app_handle.lock().as_ref().unwrap();
...
//             "toggle_tun_mode" => || feat::toggle_tun_mode(app_handle.clone()),
